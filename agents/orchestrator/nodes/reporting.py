"""
Fixed Reporting Agent - Issues corrected:
1. s3_path undefined bug
2. Better database error handling
3. Simpler INSERT without unnecessary fields
"""
...
def reporting_agent_node(state: dict) -> dict:
    ...
    run_id = state["run_id"]
    s3_path = f"{tenant_id}/{project_id}/{run_id}/report.html"
    ...
    try:
        ...
        report_url = f"{os.environ.get('S3_ENDPOINT', 'http://localhost:4566')}/{ARTIFACTS_BUCKET}/{s3_path}"
    ...
    try:
        ...
        # This is optional - don't fail the whole report if DB update fails
        print(f"[reporting-agent] note: database update skipped ({e})")
